 ...
 * 
 * Override for the default_article layout of the com_finder search results.
 * show Custom Fields with ID 86 and 71
 */
...
// Custom fields 
$CustomFields     = FieldsHelper::getFields('com_content.article', $this->result, true); // creates a variable which will contain all CF of the current article. The True will allow us to render the value instead of the rawvalue
$CustomFieldsById = \Joomla\Utilities\ArrayHelper::pivot($CustomFields, 'id'); // we use a pivot on id 
// Tags
$tags             = new TagsHelper;
$itemTags         = $tags->getItemTags('com_content.article', $this->result->id);
...
?>
<div class="jfilters result__item">
    ...
    <?php endif; ?>
    <?php // custom field kontakt ?>
    <?php if ( !empty($CustomFieldsById[86])) : ?>
        <?php $layout = $CustomFieldsById[86]->params->get('layout', 'render'); ?>
        <div class="custom_field_contact"><?php echo FieldsHelper::render('com_content.article', 'field.'. $layout .'', ['field' => $CustomFieldsById[86]]); ?></div>
    <?php endif; ?>  

    <?php // custom field downloads ?>
    <?php if ( !empty($CustomFieldsById[71])) : ?>
        <?php $layout = $CustomFieldsById[71]->params->get('layout', 'render'); ?>
        <div class="custom_field_downloads"><?php echo FieldsHelper::render('com_content.article', 'field.'. $layout .'', ['field' => $CustomFieldsById[71]]); ?></div>
    <?php endif; ?>

    <?php if ($show_description && $description !== '') :  // show readmore?>  
        <p class="readmore">  
            <a class="btn btn-default" href="<?php echo Route::_($this->result->route); ?>" aria-label="<?php echo Text::sprintf('JGLOBAL_READ_MORE_TITLE', $this->escape($this->result->title)); ?>">
    ...
    <?php if (!empty($itemTags)) : ?>
        <?php echo LayoutHelper::render('joomla.content.wbctags', $itemTags); ?>
    <?php endif ?>
